...
def register():
    ...
    if form.validate_on_submit():
        if db.session.execute(db.select(Users).where(Users.email == form.email.data)).scalar():
            flash("Already registered with this Email. Login instead")
            return redirect(url_for('login'))
        else:
            new_user = Users(name=form.name.data, email=form.email.data, #type: ignore
                             password=generate_password_hash(form.password.data, method="pbkdf2:sha256", salt_length=8)) #type: ignore
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and check_password_hash(user.password, form.password.data):
            ...
            flash("Invalid Email Address. Please register before login.")
            return redirect(url_for('login'))
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            ...
            flash("Please login or register inorder to comment")
            return redirect(url_for('login'))
...
def delete_post(post_id):
    ...
    comments_to_delete = db.session.execute(db.select(Comment).where(Comment.blog_id == post_to_delete.id)).scalars()
